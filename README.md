...
| **Asset Type** | Channel Activity Model |
| **Control Record** | Channel Activity Model Analysis |
...
| POST | `/v1/channel-activity-model-analysis/initiate` | Initiate |
| GET | `/v1/channel-activity-model-analysis` | Retrieve (list) |
| GET | `/v1/channel-activity-model-analysis/{crId}/retrieve` | Retrieve |
| PUT | `/v1/channel-activity-model-analysis/{crId}/update` | Update |
| PUT | `/v1/channel-activity-model-analysis/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/channel-activity-model-analysis/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-channel-activity-analysis:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-channel-activity-analysis ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-channel-activity-analysis` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
